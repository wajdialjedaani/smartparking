...
function Layout({ children }) {
  ...
  return (
    <div className="flex  h-screen ">
      <div className="bg-[#4EA2F0]  w-1/5 flex flex-col pt-16">
        {adminRoutes.map((route, index) => {
          return (
            <div className="px-4 py-4">
              <button className="w-full border-b-2 text-xl flex hover:bg-blue-300 font-bold justify-start p-2 "
                onClick={() => { handleNavigate(route.path) }}>
                {route.name}
              </button>
              {route.subRoutes && route.subRoutes.map((subRoute, index) => {
                return (
                  <button className="w-[93%] border-b-2 text-xl flex hover:bg-blue-300 font-bold justify-start p-2 ml-4 "
